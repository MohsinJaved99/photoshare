...
<head>
    ...
    <title>PhotoShare | Edit Admin</title>
...
if(isset($_POST['editadmin'])) {
  ...
  $editadmin="update users set `First_Name`='".$_POST['fname']."',`Last_Name`='".$_POST['lname']."',`Email`='".$_POST['email']."',`Password`='".$_POST['pass']."',`Role`='".$_POST['role']."' WHERE User_ID='".$_GET['editadmin']."'";
//   echo $addfaq;
  $editadminres=$con->query($editadmin);
    if($editadminres) {

      echo "<script> window.location='EditAdmin.php?id=".$_GET['id']."&editadmin=".$_GET['editadmin']."&Status=Success';</script>";

    }
    else {
        echo "<script> window.location='EditAdmin.php?id=".$_GET['id']."&editadmin=".$_GET['editadmin']."&Status=Failed';</script>";
    }
}
...
<h2 class="text-center" style="background-color: #c70d00;color:white;padding:20px;width: 100%;border-top-right-radius: 30px;border-top-left-radius: 30px"><span class="glyphicon glyphicon-edit"></span> Edit Admin</h2>
        ...
        if(!empty($_GET['Status'])) {
            ...
            if($_GET['Status']=="Success") {
                ...
                ?>
                    ...
                    <strong>Admin Edited!</strong> Save Changes.
                    </div>
                ...
                <button style="float: right;margin-top: -6px" class="btn btn-danger" onclick="window.location='EditAdmin.php?id=<?php echo $_GET['id'] ?>&editadmin=<?php echo $_GET['editadmin'] ?>'">Try Again.</button>
                <strong>Error!</strong> Some error occurs while editing admin.
                </div>
...
$admin="select * from users where User_ID='".$_GET['editadmin']."'";
$adminres=$con->query($admin);

foreach($adminres as $adminrow) {

?>
<h3 style="text-align: center">Admin Information</h3>
  <hr style="width: 50%">
    <form action="EditAdmin.php?id=<?php echo $_GET['id'] ?>&editadmin=<?php echo $_GET['editadmin'] ?>" method="post">

  <div class="col-lg-6">
      <label for="fname">First Name</label>
      <input id="fname" class="form-control tt" type="text" name="fname" value="<?php echo $adminrow['First_Name'] ?>">
  </div>
  <div class="col-lg-6">
      <label for="lname">Last Name</label>
      <input id="lname" class="form-control tt" type="text" name="lname" value="<?php echo $adminrow['Last_Name'] ?>">
  </div>
  <div class="col-lg-12">
      <label for="email">Email</label>
      <input id="email" class="form-control tt" type="text" name="email" value="<?php echo $adminrow['Email'] ?>">
  </div>
  <div class="col-lg-12">
      <label for="pass">Password</label>
      <input id="pass" class="form-control tt" type="text" name="pass" value="<?php echo $adminrow['Password'] ?>">
  </div>
  <div class="col-lg-12">
  <label for="role">Role</label>
  <select id="role" name="role" style="width: 100%;padding: 10px;margin-bottom: 30px;letter-spacing: 1px">
        <option value="1">Admin</option>
        <option value="3">Sub Admin</option>
  </select>
  </div>
  <input id="rol" class="form-control" type="hidden" value="<?php echo $adminrow['Role'] ?>">
  <script>
      function l() {
...
<div class="col-lg-12">
    <input type="submit" value="Save Changes" id="btn" name="editadmin">
</div>
    </form>
<?php } ?>      
    </div>
