...
<head>
    ...
    <link href="scrollbar.1css" rel="stylesheet" type="text/css">
    ...
    if(isset($_POST['login'])) {
        ...
        foreach($pendingResult as $pendingRow) {
            ...
            if($pendingRow['count']=="0") {
                ...
                $checkban="select * from users where `Email`='".$_POST["email"]."'";
                $checkbanres=$con->query($checkban);

                foreach($checkbanres as $checkbanrow) {
                    if($checkbanrow['Ban']!="True") {
                ...
                if(mysqli_num_rows($loginResult)=="1") {
                    ...
                    if($statusresult) {
                        ...
                        foreach($loginResult as $loginRow) {
                            ...
                            if($loginRow['Role']=="2") {
                                setcookie("emaill",$_POST["email"]);
                                $_SESSION['useremail']=$_POST["email"];
                                // header("location:loading.php?id=".$loginRow['User_ID']);
                                echo "<script> window.location='index.php?id=".$loginRow['User_ID']."';</script>";
                            }
                    ...
                    Swal.fire({
                    title: 'Banned!',
                    text: 'Your Account Has Been Banned By Admin, Contact mohjav031010@gmail.com For Unbaning Your Account.',
                    type: 'error'
                    }).then(function() {
                        window.location='Login.php';
                    });
            ...
            Swal.fire({
            title: 'Pending!',
            text: 'Your Email Is In Pending List Please Wait Till Admin Accept Of Reject Your Request.',
            type: 'info'
            }).then(function() {
                window.location='Login.php';
            });
...
<div class="container" id="con">
    <div class="col-lg-6 col-lg-offset-3 col-md-6 col-md-offset-3 col-sm-8 col-sm-offset-2" id="logindiv">
        <h2 class="text-center" id="lg">LOGIN</h2>
        <hr id="hr">
        <?php
        ...
        if(!empty($_GET['Ban'])) {
            ...
            if($_GET['Ban']=="True") {
                ...
                ?>
                    ...
                    <div class="alert alert-info">
                    <button style="float: right;margin-top: -6px" class="btn btn-primary" onclick="window.location='Login.php'">Ok</button>
                    <strong>Banned!</strong> Your Account Has Been Banned By Admin, Contact mohjav031010@gmail.com for unbaning account.
                    </div>
    ...
    ?>
        ...
        <form action="Login.php" method="post">
            <div class="form-group">
                <h4>Email Address</h4>
                <?php if(!empty($_COOKIE['emaill'])) { ?>
                <input id="email" class="form-control tt" type="email" name="email" value="<?php echo $_COOKIE['emaill'] ?>" placeholder="Enter Email" style="padding:20px">
                <?php }else {
                    ?>
                    <input id="email" class="form-control tt" type="email" name="email" placeholder="Enter Email" style="padding:20px" autocomplete="on">
                    <?php
                } ?>
            </div>
            <div class="form-group">
        ...
        <br>
        <h4 style="text-align: center;font-size: 15px">Don't have an account? <a href="Signup.php" style="text-decoration: underline;color:white;">Join</a></h4>
