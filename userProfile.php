...
<head>
    ...
    <?php 
                    $user1="select * from users where User_ID='".$_GET['userid']."'";
                    $resuser1=$con->query($user1);
                    foreach($resuser1 as $rowuser1) {
            ?>
    <title>PhotoShare | <?php echo $rowuser1['First_Name'] ?>'s Profile</title>
                    <?php  } ?>
...
/* #fiv #rp {
  ...
  margin-top: 10px
            ...
            #mgal {
              font-weight: bold;letter-spacing: 1px;font-family: 'calibri';text-align: center;margin-top: 0px
            }

            #fiv #p {
              color:white;margin-top: 70px;font-family: 'calibri';
            }
@media only screen and (max-width: 600px) {
          ...
          #mgal {
              font-weight: bold;letter-spacing: 1px;font-family: 'calibri';text-align: center;width: 100%;
            }
...
<div class="col-lg-12" style="margin-top: 50px;margin-bottom: 100px">
        ...
        <div class="col-lg-8 col-lg-offset-2 col-md-10 col-md-offset-1">
            <?php 
                    $user="select * from users where User_ID='".$_GET['userid']."'";
            ...
            <div class="col-lg-8 col-md-8 col-sm-7">
              ...
              $getfollow="select count(*) as count from follow where Following_User='".$_GET['userid']."' and Follow_ByUser='".$_GET['id']."'";
...
foreach($getfollowres as $getfollowrow) {
    if($getfollowrow['count']==0) {
              ...
              ?>
              <button class="btn btn-default" style="float: right;letter-spacing: 1px" id="follow">Follow</button>
    <?php }
    else {
     ?>
...
<div class="dropdown" style="float: right">
  <button class="btn btn-default dropdown-toggle" type="button" data-toggle="dropdown">
    Following <span class="caret"></span></button>
  <ul class="dropdown-menu" style="margin-left: -63px;">
  <li id="follow"><a href="#">Unfollow</a></li>
  </ul>
</div>
    ...
    <?php if($rowuser['Registered']=="True") { ?>
                    <span class="glyphicon glyphicon-ok-sign" title="Registered User Badge" style="color:#fc6100;font-size: 25px"></span>
                    <?php } ?></h1>
  ...
  $countfollowing="select count(*) as count from follow where Follow_ByUser='".$_GET['userid']."'";
  $countfollowingres=$con->query($countfollowing);

    foreach($countfollowingres as $countfollowingrow){
  ?>
    <h5><span class="glyphicon glyphicon-flash"></span>  Following <strong><?php echo thousandsCurrencyFormat($countfollowingrow['count']); ?></strong></h5>
  <?php 

    } ?>
    <hr>
    
    <h4><span class="glyphicon glyphicon-globe" style="color:#fc6100"></span> <span id="fl"></span></h4>
    <h4><span class="glyphicon glyphicon-heart" style="color:#fc6100"></span> <span id="lk"></span> Likes</h4>
    <input type="hidden" value="<?php echo $_GET['userid'] ?>" id="uid">
    <h4><span class="glyphicon glyphicon-download" style="color:#fc6100"></span> <span id="dwl"></span> Downloads</h4>

    <?php
   
   $totalc="select count(*) as count from images where `Image_ByUser`='".$_GET['userid']."'";
...
<h1 id="mgal"><?php echo $rowuser['First_Name'] ?>'s Gallery</h1><br><br><br>
    <div class="grid-padding">

        <div class="row">
        <?php 
    $countimg="select count(*) as counts from images where Image_ByUser='".$_GET['userid']."'";
...
<?php 
    ...
    $getimg="select * from images where Image_ByUser='".$_GET['userid']."' order by Image_ID DESC";
...
<input type="hidden" value="<?php echo $_GET['id'] ?>" id="myid">
<input type="hidden" value="<?php echo $_GET['userid'] ?>" id="user">
<script>
...
var userid=document.getElementById('user').value;
  $.ajax({
              type: "post",
                    ...
                    success: function (data1) {
                      if(data1=="1") {
                        $('#fl').html(data1+" Follower");
                      }
                      else {
                        $('#fl').html(data1+" Followers");
                      }
...
$.ajax({
        type : 'post',
        url : 'follow.php', //Here you will fetch records 
        data :  {id:id,userid:userid}, //Pass $id
        success : function(data){
          
                    if(data=="Unfollow") {
                      window.location="userProfile.php?id="+id+"&userid="+userid;
                      // Swal.fire('Unfollow.');
...
<div class="col-lg-12" style="margin-top: 50px;margin-bottom: 100px">
  ...
  <?php 
    ...
    <hr>
    <h4><span class="glyphicon glyphicon-globe" style="color:#fc6100"></span> <span id="fll"></span></h4>
    <h4><span class="glyphicon glyphicon-heart" style="color:#fc6100"></span> <span id="lk"></span> Likes</h4>
    <input type="hidden" value="<?php echo $_GET['userid'] ?>" id="uid">
    <h4><span class="glyphicon glyphicon-download" style="color:#fc6100"></span> <span id="dwl"></span> Downloads</h4>
